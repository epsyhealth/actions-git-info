...
def info(pr, debug, work_dir):
    ...
    for k, v in variables.items():
        ...
        click.echo(
            f"{k}={v if type(v) != bool else str(v).lower()}", os.getenv("GITHUB_OUTPUT")
        )
...
def has_changes(work_dir, debug):
    ...
    click.echo(
        f"has_changes={'true' if repo.is_dirty() else 'false'}", os.getenv("GITHUB_OUTPUT")
    )
...
def last_commit(debug, days):
    ...
    click.echo(f"last_commit={diff.days <= days}", os.getenv("GITHUB_OUTPUT"))
...
def is_behind(pr, work_dir, debug):
    ...
    click.echo(f"is_behind={str(is_behind).lower()}", os.getenv("GITHUB_OUTPUT"))
